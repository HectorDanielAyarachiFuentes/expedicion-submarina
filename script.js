(function(){
  ...
  let icoMuteURL=null,icoInfoURL=null,icoFSURL=null,icoShareURL=null; let grayExitURL=null,grayContinueURL=null,grayDiveURL=null,grayRetryURL=null,grayDonateURL=null;
  ...
  const S=(function(){ let created=false; const a={}; let _muted=false; const srcMap={ fire:RAW+'sfx_fire.mp3', lose:RAW+'sfx_lose.mp3', gameover:RAW+'sfx_gameover.mp3', music:MUSIC, torpedo: 'https://cdn.freesound.org/previews/219/219244_4023635-lq.mp3' };
  ...
  // ========= Particles & Effects =========
  let particles=[]; let explosionParticles=[];
  ...
  function updateParticles(dt) {
      for(let arr of [particles, explosionParticles]) {
  ...
  function drawParticles() {
      ...
      for(const p of particles) { ctx.globalAlpha = clamp(p.baseA * (0.65 + 0.35 * Math.sin(p.tw)), 0, 1); ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fillStyle = p.color; ctx.fill(); }
  ...
  const TORPEDO_COOLDOWN = 2.0;
  let torpedos = [];

  function reset(){
    state={ run:false, rescued:0, score:0, depth_s:0, lives:3, lifeAnim:0, spawn:0, speed:260, elapsed:0, inputLock:0.2,
  ...
  function currentSpawnPeriod(){ const p0=clamp(difficultyRaw(),0,1); let base=lerp(2.5,0.6,p0); let period=base*lerp(2.0,1.0,p0); const p=difficultyRaw(); if(p>1){ period=Math.max(0.3,period/(1+0.7*(p-1)));} return period; }
  ...
  function update(dt){
    ...
    for (let i = torpedos.length - 1; i >= 0; i--) {
        const t = torpedos[i];
        t.x += 1200 * dt;
        if (t.x > W) { torpedos.splice(i, 1); continue; }
  ...
  function render(dt){
    // Background
    if (state && state.run) drawBG(dt);
    
    // Main Canvas
    ctx.clearRect(0,0,W,H);
    if (state && state.run) {
        for(let i=0;i<animals.length;i++){ const a=animals[i]; const floatOffset=Math.sin(Math.PI*state.elapsed + a.phaseSeed)*5; if(creaturesReady&&cRows>0){ const sx=(a.frame%2)*cFrameW, sy=(a.row%cRows)*cFrameH; ctx.imageSmoothingEnabled=false; ctx.drawImage(creaturesImg,sx,sy,cFrameW,cFrameH, Math.round(a.x-a.size/2), Math.round(a.y+floatOffset-a.size/2), a.size,a.size); } else { ctx.fillStyle='#ffd95e'; ctx.beginPath(); ctx.arc(a.x,a.y+floatOffset,a.r,0,Math.PI*2); ctx.fill(); } }
  ...
  function drawBG(dt){ if (!state) return; if(bgReady&&bgW&&bgH){ const spd=BG_BASE_SPEED*(1+0.6*clamp(difficultyRaw(),0,2)); bgOffset=(bgOffset+spd*dt)%bgW; bgCtx.setTransform(1,0,0,1,0,0); bgCtx.clearRect(0,0,W,H); bgCtx.imageSmoothingEnabled=false; let startX=-Math.floor(bgOffset)-bgW; for(let x=startX; x<W+bgW; x+=bgW){ for(let y=0; y<H+bgH; y+=bgH){ bgCtx.drawImage(bgImg, Math.round(x), Math.round(y)); } } } else { bgCtx.clearRect(0,0,W,H); } if(fgReady&&fgW&&fgH){ const fspd=FG_BASE_SPEED*(1+0.6*clamp(difficultyRaw(),0,2)); fgOffset=(fgOffset+fspd*dt)%fgW; const yBase=H-fgH; const startXF=-Math.floor(fgOffset)-fgW; for(let xx=startXF; xx<W+fgW; xx+=fgW){ bgCtx.drawImage(fgImg, Math.round(xx), Math.round(yBase)); } } }
  function drawLightMask(){ if (!state) return; fx.clearRect(0,0,W,H); const alpha=lerp(0,0.9, clamp((state?state.elapsed:0)/180,0,1)); if(alpha<=0.001) return; fx.globalCompositeOperation='source-over'; fx.fillStyle='rgba(0,0,0,'+alpha.toFixed(3)+')'; fx.fillRect(0,0,W,H); if(state.lightVisible && player){ const px=player.x*W + bobX(), py=player.y*H; const ang=robotTilt; const ux=Math.cos(ang), uy=Math.sin(ang); const vx=-Math.sin(ang), vy=Math.cos(ang); const ax=Math.round(px + ux*(spriteW*robotScale*0.5 - 11) + vx*(-4)); const ay=Math.round(py + uy*(spriteW*robotScale*0.5 - 11) + vy*(-4)); const L=Math.min(W*0.65,560); const theta=Math.PI/9; const endx=ax+ux*L, endy=ay+uy*L; const half=Math.tan(theta)*L; const pTopX=endx+vx*half, pTopY=endy+vy*half; const pBotX=endx-vx*half, pBotY=endy-vy*half; let g=fx.createLinearGradient(ax,ay,endx,endy); g.addColorStop(0.00,'rgba(255,255,255,1.0)'); g.addColorStop(0.45,'rgba(255,255,255,0.5)'); g.addColorStop(1.00,'rgba(255,255,255,0.0)'); fx.globalCompositeOperation='destination-out'; fx.fillStyle=g; fx.beginPath(); fx.moveTo(ax,ay); fx.lineTo(pTopX,pTopY); fx.lineTo(pBotX,pBotY); fx.closePath(); fx.fill(); const rg=fx.createRadialGradient(ax,ay,0, ax,ay,54); rg.addColorStop(0,'rgba(255,255,255,1.0)'); rg.addColorStop(1,'rgba(255,255,255,0.0)'); fx.fillStyle=rg; fx.beginPath(); fx.arc(ax,ay,54,0,Math.PI*2); fx.fill(); fx.globalCompositeOperation='lighter'; const gGlow=fx.createLinearGradient(ax,ay,endx,endy); gGlow.addColorStop(0.00,'rgba(255,255,255,0.14)'); gGlow.addColorStop(0.60,'rgba(255,255,255,0.06)'); gGlow.addColorStop(1.00,'rgba(255,255,255,0.00)'); fx.fillStyle=gGlow; fx.beginPath(); fx.moveTo(ax,ay); fx.lineTo(pTopX,pTopY); fx.lineTo(pBotX,pBotY); fx.closePath(); fx.fill(); fx.globalCompositeOperation='source-over'; } }
  function drawHUD(){ if (!state) return; hud.clearRect(0,0,W,H); if (!state.run) return; const s=state, depthVal=Math.floor(s.depth_s||0), scoreVal=s.score||0, livesVal=s.lives||3, rescuedVal=s.rescued||0; const padX=18,padY=18,lh=22; const rows=[{label:'SCORE',value:String(scoreVal)},{label:'DEPTH',value:depthVal+' m'},{label:'RECORD',value:String(highScore)}]; const totalRows=rows.length+3; const y0=H-padY-lh*totalRows; hud.save(); hud.fillStyle='#ffffff'; hud.font='18px "Press Start 2P", monospace'; hud.textAlign='left'; hud.textBaseline='alphabetic'; let maxLabelW=0; for(let i=0;i<rows.length;i++) maxLabelW=Math.max(maxLabelW, hud.measureText(rows[i].label).width); const gap=16; const valueX=padX+maxLabelW+gap; for(let i=0;i<rows.length;i++){ const y=y0+i*lh; hud.fillText(rows[i].label, padX, y); hud.fillText(rows[i].value, valueX, y); } const livesY=y0+rows.length*lh; hud.fillText('LIVES', padX, livesY); hud.fillStyle='#ff4d4d'; hud.fillText('♥'.repeat(livesVal)+'♡'.repeat(3-livesVal), valueX, livesY); hud.fillStyle='#ffffff'; hud.fillText('CAPTURES', padX, livesY+lh); hud.fillText(String(rescuedVal), valueX, livesY+lh); const torpedoY = livesY + lh * 2; hud.fillText('TORPEDO', padX, torpedoY); const torpedoReady = s.torpedoCooldown <= 0; hud.fillStyle = torpedoReady ? '#66ff66' : '#ff6666'; hud.fillText(torpedoReady ? 'LISTO' : 'RECARGANDO...', valueX, torpedoY); const barW = 100, barH = 10; const barX = valueX + hud.measureText('RECARGANDO...').width + 10; hud.fillStyle = 'rgba(255,255,255,0.2)'; hud.fillRect(barX, torpedoY - barH, barW, barH); const progress = clamp(1 - (s.torpedoCooldown / TORPEDO_COOLDOWN), 0, 1); hud.fillStyle = '#66ff66'; hud.fillRect(barX, torpedoY - barH, barW * progress, barH); hud.fillStyle = '#ffffff'; const iso=new Date().toISOString(); const utcStr=iso.slice(0,10)+' '+iso.slice(11,19)+' UTC'; hud.textAlign='center'; hud.fillText(utcStr, W/2, H-12); hud.restore(); }
  
  // ========= Main Loop Setup =========
  let last=0; function loop(t){ const dt=Math.min(0.033,(t-last)/1000||0); last=t; update(dt); render(dt); requestAnimationFrame(loop); }

  // ========= Start / End =========
  let __starting=false; function start(){ if(__starting) return; __starting=true; if(state&&state.run){ __starting=false; return; } reset(); keys={}; state.inputLock=0.2; state.run=true; state.spawn=0.01; S.init(); S.stop('music'); S.loop('music'); titleEl.style.display='none'; brandLogo.style.display='block'; finalP.innerHTML='Captura tantos especímenes<br/>como puedas'; finalStats.style.display='none'; if(mainExtras) mainExtras.style.display='none'; overlay.style.display='none'; overlayMode='menu'; startBtn.style.display='inline'; restartBtn.style.display='none'; refreshIcons(); setTimeout(function(){ __starting=false; },200); }
  ...
  function showMainMenuView(fromPause){ brandLogo.style.display='block'; titleEl.style.display='none'; finalP.innerHTML='Captura tantos especímenes<br/>como puedas'; finalStats.style.display='none'; const divider=document.getElementById('menuDivider'); if(divider) divider.style.display='block'; if(mainExtras) mainExtras.style.display= fromPause ? 'block':'none'; setModalButtons(fromPause); startBtn.style.display='inline'; restartBtn.style.display = fromPause ? 'inline' : 'none'; overlayMode= fromPause ? 'pause':'menu'; overlay.style.display='grid'; }
  ...
  if(shareBtn){ shareBtn.onclick = async function(){ let wasRunning = !!(state && state.run); if(wasRunning){ state.run=false; S.pause('music'); } try{ if(navigator.share){ await navigator.share({ title:'Expedición Mardel', text:'Captura tantos especímenes del Cañón de Mar del Plata como puedas', url: location.href }); } }catch(_){} finally{ if(wasRunning && overlay.style.display==='none'){ state.run=true; S.loop('music'); } } }; }
  overlay.addEventListener('click', function(e){ if(e.target===overlay && overlay.style.display!=='none' && restartBtn.style.display==='none'){ if(overlayMode==='pause'){ overlay.style.display='none'; if(state){ state.run=true; state.inputLock=0.15; } S.loop('music'); } else { start(); } } });
